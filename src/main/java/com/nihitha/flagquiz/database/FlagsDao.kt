package com.nihitha.flagquiz.database
...
class FlagsDao {
    fun getRandomTenRecords(helper:DatabaseCopyHelper):ArrayList<flagModel>{
        ...
        while(cursor.moveToNext()){
            val record = flagModel(cursor.getInt(idIndex),cursor.getString(countryNameIndex),cursor.getString(flagNameIndex))
    ...
    fun getRandomThreeRecords(helper:DatabaseCopyHelper,id:Int):ArrayList<flagModel>{
        val recordList = ArrayList<flagModel>()
        val database:SQLiteDatabase = helper.writableDatabase
